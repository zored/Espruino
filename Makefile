...
ifeq ($(FAMILY), NRF51)
 ...
 ARCHFLAGS = -mthumb -mabi=aapcs -mcpu=cortex-m0 -mfloat-abi=soft
 
 ifdef BLE_INTERFACE
  DEFINES += -DSWI_DISABLE0 -DSWI_DISABLE3 -DSOFTDEVICE_PRESENT -DNRF51 -DBOARD_PCA10028 -DS110 -DBLE_STACK_SUPPORT_REQD -DSVCALL_AS_NORMAL_FUNCTION -DSPI_MASTER_0_ENABLE
  LINKER_FILE = $(NRF5X_SDK_PATH)/components/toolchain/gcc/linker_nrf51_ble_interface_espruino.ld
 else
  DEFINES += -DCONFIG_GPIO_AS_PINRESET -DBOARD_PCA10028 -DNRF51 -DBSP_DEFINES_ONLY
...
ifeq ($(FAMILY), NRF52)
 ...
 ifdef BLE_INTERFACE
  ...
  LINKER_FILE = $(NRF5X_SDK_PATH)/components/toolchain/gcc/linker_nrf52_ble_interface_espruino.ld
 else
...
ifdef NRF5X
 ARM=1
 ARM_HAS_OWN_CMSIS=1 # Nordic uses its own CMSIS files in its SDK. These are the most recent CMSIS files.
 ...
 ifdef BLE_INTERFACE
  SOURCES +=       \
  targets/nrf5x/ble_interface.c
 else
  SOURCES +=       \
  targets/nrf5x/uart_interface.c
 endif # BLE_INTERFACE
 ...
 ifdef BLE_INTERFACE
  ...
  INCLUDE += -I$(NRF5X_SDK_PATH)/examples/peripheral/uart/config/uart_pca10036
  INCLUDE += -I$(NRF5X_SDK_PATH)/examples/peripheral/uart/config
  INCLUDE += -I$(NRF5X_SDK_PATH)/examples/bsp
  INCLUDE += -I$(NRF5X_SDK_PATH)/components/drivers_nrf/nrf_soc_nosd
  INCLUDE += -I$(NRF5X_SDK_PATH)/components/device
  INCLUDE += -I$(NRF5X_SDK_PATH)/components/libraries/uart
  INCLUDE += -I$(NRF5X_SDK_PATH)/components/drivers_nrf/hal
  INCLUDE += -I$(NRF5X_SDK_PATH)/components/drivers_nrf/delay
  INCLUDE += -I$(NRF5X_SDK_PATH)/examples/peripheral/uart
  INCLUDE += -I$(NRF5X_SDK_PATH)/components/libraries/util
  INCLUDE += -I$(NRF5X_SDK_PATH)/components/drivers_nrf/uart
  INCLUDE += -I$(NRF5X_SDK_PATH)/components/drivers_nrf/common
  INCLUDE += -I$(NRF5X_SDK_PATH)/components/toolchain
  ...
  INCLUDE += -I$(NRF5X_SDK_PATH)/examples/peripheral/timer/config/timer_pca10036
  INCLUDE += -I$(NRF5X_SDK_PATH)/examples/peripheral/timer/config
  INCLUDE += -I$(NRF5X_SDK_PATH)/components/drivers_nrf/timer
  INCLUDE += -I$(NRF5X_SDK_PATH)/examples/peripheral/timer
  ...
  $(NRF5X_SDK_PATH)/components/drivers_nrf/common/nrf_drv_common.c \
  $(NRF5X_SDK_PATH)/components/drivers_nrf/uart/nrf_drv_uart.c \
  $(NRF5X_SDK_PATH)/components/drivers_nrf/timer/nrf_drv_timer.c # I want to add timer peripheral so add source here (as in timer example makefile from nordic sdk).
...
ifndef NRF5X # NRF51/NRF52 platform uses its own linker file that isnt automatically generated.
